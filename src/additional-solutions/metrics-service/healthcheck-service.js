...
exports.healthcheck = async function () {
    ...
    requestOpts = {
        ...
        headers: { 'x-v': 1 },
        body: null,
        uri: apigeeConfig.default_base_uri + '/cds-au/v1/discovery/status',
    ...
    request(requestOpts, async function (err, res, body) {
        defaultOptions.debug && console.log("After receiving response....");
        defaultOptions.debug && console.log("Received response: " + JSON.stringify(res));
        // Parse response
        if (err) {
            ...
            if (res.statusCode == 200) {
                ...
                if ((parsed_body) && (parsed_body.errors)) {
                    status_reason = JSON.stringify(parsed_body.errors);
                } else {
